...
class RuleEditorPanel {
    constructor(panel, store, editingRule) {
        this.store = store;
    ...
    static show(store, rule) {
        ...
        const panel = vscode.window.createWebviewPanel('paycontextRuleEditor', rule ? `编辑规则: ${rule.name}` : '新建规则', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    getHtml() {
        ...
        const categories = Object.entries(types_1.CATEGORY_LABELS);
        const categoryOptions = categories
            .map(([value, label]) => {
            const selected = r?.category === value ? 'selected' : '';
            return `<option value="${value}" ${selected}>${types_1.CATEGORY_ICONS[value]} ${label}</option>`;
        })
            .join('\n');
...
//# sourceMappingURL=ruleEditorPanel.js.map